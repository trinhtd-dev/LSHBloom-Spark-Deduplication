...
class BlowChocFilter:
    """
    BlowChoc (BLoom filter With Optimal Hashing for Cache-line aligned Optimal Collisions).
    ...
    - BlowChoc: nhồi toàn bộ k bits vào đúng 1 word 64-bit duy nhất được chọn ngẫu nhiên.
    ...
    def __init__(self, n: int, fp_rate: float = 1e-10):
        ...
        # BlowChoc penalty nhỏ hơn Blocked Bloom nhiều (~1.05-1.1x)
    ...
    def _correction_factor(k: int) -> float:
        """
        BlowChoc FPR penalty nhỏ hơn Blocked Bloom đáng kể.
        Vì toàn bộ k bits nằm trong 1 word 64-bit, phân bố đều hơn
        so với rải trên cả 512-bit block.
        """
        # Xấp xỉ từ phân tích lý thuyết BlowChoc (Putze et al. 2010)
        table = {
    ...
    def _get_word_and_bits(self, item: int):
        """
        BlowChoc core: chọn 1 block, sau đó chọn 1 word, rồi nhồi k bits vào word đó.
...
class BlowChocTable:
    def __init__(self, item_count: int, fp: float, num_arrays: int, fname: str = None, max_size: int = None):
        ...
        if fname is not None and (os.path.exists(fname) or npy_exists):
            print(f"Loading BlowChoc Filter at {fname}...")
            self.bloom_filter = BlowChocFilter.load(fname, item_count, fp)
        else:
            self.bloom_filter = BlowChocFilter(n=item_count, fp_rate=fp)
...
class MinHashLSHBlowChoc(object):
    def __init__(
        self,
        threshold: float = 0.9,
        num_perm: int = 128,
        ...
        self.hashtables = [
            BlowChocTable(
                ...
                fp=fp,
                num_arrays=self.r,
                fname=os.path.join(save_dir, f"band-{i}.bc") if save_dir is not None else None,
            )
    ...
    def sync(self):
        print("Saving BlowChoc Index...")
        for table in self.hashtables:
            table.sync()


class LSHBlowChocDeduper(DedupHarness):
    def __init__(self, n: int, sim_threshold: float, num_perm: int, minhash_root: str, save_dir: str,
        ...
        if clear_filter_state:
            for item in glob(f"{save_dir}/*.bc.npy"):
                os.remove(item)
                print(f"Clearing BlowChoc filter: {item}")
        p_effective = fp
        b, r = _optimal_param(self.T, self.k, 0.5, 0.5)
        fp_optimal = 1.0 - (1.0 - p_effective) ** (1.0 / b)
        self.lsh = MinHashLSHBlowChoc(threshold=self.T, num_perm=self.k, fp=fp_optimal, n=self.n, save_dir=self.save_dir)
...
if __name__ == "__main__":
    ...
    result_dir = os.path.join(WORK_DIR, benchmark_tag, f"lsh_blowchoc_results{fp_tag}")
    minhash_root = os.path.join(result_dir, "minhashes", f"{args.num_perm}")
    save_dir = os.path.join(result_dir, f"blowchoc_filter_{args.sim_threshold}_{args.num_perm}")
    output_file = os.path.join(result_dir, f"lsh_blowchoc_{args.sim_threshold}_{args.num_perm}_preds.csv")
    result_file = os.path.join(result_dir, f"lsh_blowchoc_{args.sim_threshold}_{args.num_perm}_score.csv")
