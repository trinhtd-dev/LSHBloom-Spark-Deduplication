...
from dolma_deduper import DolmaDedup
...
def get_args():
    ...
    parser.add_argument(
		"--ngram-mode",
		help="Whether to use ngram mode as opposed to paragraph level ingestion",
		action='store_true'
	)
    parser.add_argument(
		"--ngram-size",
...
class DolmaDeduper(DedupHarness):
    def __init__(self, n: int, sim_threshold: float, ngram_mode: bool, ngram_size: int, save_dir: str):
        name = "dolma" if not ngram_mode else f"dolma_ngram_{ngram_size}"
        super().__init__(name)
        ...
        self.deduper = DolmaDedup(n=self.n, fp=0.00001, T=self.T, save_file=self.save_file)

    def deduplicate(self, text: str, id: int) -> bool:
        is_dup = self.deduper.dedup_single(text)
        return is_dup
...
if __name__ == "__main__":
    ...
    name = "dolma" if not args.ngram_mode else "dolma_ngram"
    run_id = f"{args.sim_threshold}" if not args.ngram_mode else f"{args.sim_threshold}_{args.ngram_size}"
    ...
    if args.ngram_mode:
       with open(os.path.join(WORK_DIR, benchmark_tag, f"ngram_count_{args.ngram_size}.txt"), 'r') as f:
        ...
        with open(os.path.join(WORK_DIR, benchmark_tag, f"para_count.txt"), 'r') as f:
           N = int(f.readline())

    deduper = DolmaDeduper(
        n=N, 
        sim_threshold=float(args.sim_threshold), 
        ngram_mode=bool(args.ngram_mode), 
