...
def minhash_lsh_runner(
    ...
    batch_rows: int,
    workers: int,
    clear_filter_state: bool = False,  # accepted but unused (no persistent state to clear)
):
    """
    MinHash LSH — candidate-only dedup (paper-correct):
      build_sig_sec : parallel MinHash via Pool(workers)
      query_sec     : LSH lookup in hash table → any hit = duplicate
      insert_sec    : insert unique doc into index

    Storage backend: Python dict keyed by (band_hash → list[doc_id]).
    Memory grows O(N); compare with lsh_bloom's O(1) bloom filter.
    """
    ...
    try:
        for table in iter_text_tables(
            ...
            # build signatures in parallel — same hash fn as lsh_bloom for fair comparison
            if pool is not None:
            ...
            for (doc_id, _), (hashvalues, n_tokens, n_shingles) in zip(doc_texts, payloads):
                ...
                # query: any candidate hit from hash table → treat as duplicate
                t0 = time.perf_counter()
                candidates = lsh.query(mh)
                query_sec += time.perf_counter() - t0

                real_candidates = [int(c) for c in candidates if int(c) != doc_id]
                is_dup = len(real_candidates) > 0

                if is_dup:
                    docs_with_hits += 1
                    total_hit_count += len(real_candidates)
                else:
                    t0 = time.perf_counter()
