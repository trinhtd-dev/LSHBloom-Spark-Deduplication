...
from dedup.lsh.lsh_blowchoc import MinHashLSHBlowChoc, _optimal_param
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Standalone Spark benchmark for LSH BlowChoc following paper semantics.")
    parser.add_argument("--input", required=True, help="Input parquet directory with part-*.parquet files")
    parser.add_argument("--output-dir", default="spark_blowchoc_runs", help="Output directory for metrics and artifacts")
    ...
    parser.add_argument("--app-name", default="LSHBlowChocBenchmark")
...
def make_run_id(args: argparse.Namespace) -> str:
    return f"blowchoc_np{args.num_perm}_t{args.threshold}_ng{args.ngram}_bfp{args.bloom_fp}".replace(".", "p")
...
class SparkBlowChocBenchmark:
    """Standalone benchmark wrapper that mimics the style of Spark ML benchmark files.

    This is intentionally independent: it includes text preprocessing, MinHash
    creation, BlowChoc state management, metrics collection, and output writing.
    """

    def __init__(self, threshold: float, num_perm: int, bloom_fp: float, bloom_n: int, save_dir: str, ngram: int = 5, max_unique: int = 1024):
        # Tính fp_optimal cho từng band theo công thức chuẩn của paper:
        # nếu target overall FPR = bloom_fp thì mỗi band cần fp = 1 - (1-bloom_fp)^(1/b)
        b, _ = _optimal_param(threshold, num_perm, 0.5, 0.5)
        fp_optimal = 1.0 - (1.0 - bloom_fp) ** (1.0 / b)

        self.deduper = MinHashLSHBlowChoc(
            threshold=threshold,
            num_perm=num_perm,
            fp=fp_optimal,
            n=bloom_n,
    ...
    def build_index(self, doc_df: DataFrame) -> tuple[DataFrame, DataFrame]:
        ...
        def _build_mh(text: str):
            mh, _, _ = build_minhash_from_text(text, self.num_perm, self.ngram, self.max_unique)
    ...
    def generate_candidates(self, rows: Iterable[dict], stats: TimingStats, mem: ProcessMemoryMonitor):
        ...
        for row in rows:
            ...
            # 1. Xây dựng chữ ký MinHash
            t0 = time.perf_counter()
            _mh, _, _ = build_minhash_from_text(text, self.num_perm, self.ngram, self.max_unique)
            stats.build_signature_sec += time.perf_counter() - t0

            # 2. Query trạng thái trùng lặp trong BlowChoc LSH
            t0 = time.perf_counter()
            is_dup = self.deduper.query(_mh)
            stats.query_sec += time.perf_counter() - t0

            # 3. Nếu duy nhất, insert vào BlowChoc LSH để các doc sau có thể đối chiếu
...
def main() -> int:
    ...
    # Tính bloom_n động từ Spark để BlowChoc filter có kích thước bit chuẩn xác
    total_docs = spark.read.parquet(args.input).count()
    bloom_n = args.bloom_n if args.bloom_n and args.bloom_n > 0 else total_docs
    print(f"[blowchoc] total_docs={total_docs} bloom_n={bloom_n}")

    filter_dir = run_dir / "blowchoc_filter"
    filter_dir.mkdir(parents=True, exist_ok=True)

    benchmark = SparkBlowChocBenchmark(
    ...
    print(f"[blowchoc] start threshold={args.threshold} num_perm={args.num_perm} workers={args.workers}")
    rows = []
    batch: list[dict] = []
    batch_idx = 0
    ...
    for batch_rows in rows:
        print(f"[blowchoc] processing batch={batch_idx:05d} size={len(batch_rows)}")
        batch_result = benchmark.generate_candidates(batch_rows, timing, mem)
        # persist per-batch outputs for reproducibility
        ...
        if batch_idx % 5 == 0:
            ...
            print(
                f"[blowchoc] batches={batch_idx} docs={timing.total_docs_seen} "
                f"build={timing.build_signature_sec:.1f}s query={timing.query_sec:.1f}s "
                f"insert={timing.insert_sec:.1f}s elapsed={elapsed:.1f}s"
            )

    # Lưu mảng bit BlowChoc xuống đĩa (sync thay vì teardown)
    if hasattr(benchmark.deduper, "sync"):
        benchmark.deduper.sync()
    wall_clock_sec = time.perf_counter() - start_wall

    result = {
        "algo_name": "lsh_blowchoc",
        "run_id": run_id,
        "input": args.input,
        "threshold": args.threshold,
        "num_perm": args.num_perm,
        ...
        "bloom_fp": args.bloom_fp,
        "bloom_n": bloom_n,  # lưu giá trị thực tế đã dùng
